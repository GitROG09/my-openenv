...
Referenced by:
  ...
  [tool.openenv]     app_file = "server/app.py"

This module is self-contained — it does NOT import from the root app.py,
which avoids the circular import that occurs when gunicorn loads app:app
and app.py is already partially initialised.
"""

import os
import sys

# Ensure repo root is on sys.path so env.py, tools.py, etc. are importable
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

from flask import Flask, jsonify, request, render_template_string  # noqa: E402
from env import ToolOrchestrationEnv   # noqa: E402
from baseline import BaselineAgent     # noqa: E402
...
_env             = ToolOrchestrationEnv()
_baseline_agent  = BaselineAgent()


# ---------------------------------------------------------------------------
# Dashboard HTML  (same as root app.py)
# ---------------------------------------------------------------------------
...
.card { background: white; color: black; padding: 25px; margin-bottom: 20px;
        border-radius: 14px; box-shadow: 0 10px 30px rgba(0,0,0,0.25); }
.step-title { font-size: 18px; font-weight: 700; margin-bottom: 14px; }
button { background: #4c6ef5; border: none; color: white; padding: 11px 22px;
         border-radius: 8px; font-size: 15px; cursor: pointer; transition: background 0.15s; }
button:hover  { background: #364fc7; }
button:disabled { background: #adb5bd; cursor: not-allowed; }
select, input[type=text] { padding: 10px 12px; border-radius: 8px;
  border: 1px solid #ced4da; font-size: 14px; margin-right: 8px;
  outline: none; transition: border-color 0.15s; }
select:focus, input[type=text]:focus { border-color: #4c6ef5; }
.goal-box { background: #f1f3f5; padding: 15px; border-radius: 10px;
            font-size: 13px; margin-top: 14px; white-space: pre-wrap;
            font-family: monospace; min-height: 48px; color: #333; }
.timeline { background: #1a1a2e; color: #00ff9c; padding: 15px;
...
.reward-row { display: flex; gap: 30px; flex-wrap: wrap; align-items: center; }
...
.steps-bar-fill  { background: #4c6ef5; height: 100%; width: 0%;
...
<div class="container">
  ...
  <div class="card">
    <div class="step-title">1️⃣ Select Mission
  ...
  <div class="card">
    ...
    <div class="steps-bar-wrap">
      ...
      <div class="steps-bar-bg"><div class="steps-bar-fill" id="stepsBar"></div></div>
    </div>
  </div>
  <div class="card">
...
<script>
let episodeStarted=false,episodeDone=false,maxSteps=0,stepCount=0;
updatePlaceholder();
function updatePlaceholder(){
  const tool=document.getElementById('toolSelect').value;
  const input=document.getElementById('paramsInput');
  const hint=document.getElementById('inputHint');
  if(tool==='get_weather'){input.placeholder='Mumbai';hint.innerHTML='Enter a city name, e.g. <strong>Mumbai</strong> or <strong>Delhi</strong>';}
  else if(tool==='search_flights'){input.placeholder='Pune, Delhi';hint.innerHTML='Enter <strong>Source, Destination</strong> separated by a comma — e.g. <strong>Pune, Delhi</strong>';}
  else{input.placeholder='AI101';hint.innerHTML='Enter a flight number, e.g. <strong>AI101</strong> or <strong>SG202</strong>';}
}
function setBadge(s){
  const b=document.getElementById('episodeBadge');b.className='badge';
  if(s==='running'){b.classList.add('badge-running');b.textContent='✅ Running';}
  else if(s==='done'){b.classList.add('badge-done');b.textContent='🏁 Done';}
  else{b.classList.add('badge-idle');b.textContent='Not started';}
}
...
async function resetEnv(){
  ...
  try{
    ...
    episodeStarted=true;episodeDone=false;maxSteps=data.remaining_steps;stepCount=0;
    ...
    setBadge('running');updateStepsBar(0,maxSteps);
    setRewardEl('stepReward','—');setRewardEl('totalReward',0);setRewardEl('normScore',0);
  }catch(err){alert('Network error: '+err.message);}
}
async function executeStep(){
  if(!episodeStarted){alert('Please click "▶ Start Episode" first.');return;}
  if(episodeDone){alert('Episode finished. Click "▶ Start Episode" to begin a new one.');return;}
  const tool=document.getElementById('toolSelect').value;
  const raw=document.getElementById('paramsInput').value.trim();
  if(!raw){alert('Please type a value in the input field first.');return;}
  let params={};
  if(tool==='get_weather'){params={city:raw};}
  else if(tool==='search_flights'){
    const parts=raw.split(',');
    if(parts.length!==2||!parts[0].trim()||!parts[1].trim()){alert('Enter Source, Destination — e.g. Pune, Delhi');return;}
    params={source:parts[0].trim(),destination:parts[1].trim()};
  }else{params={flight_number:raw};}
  const btn=document.getElementById('executeBtn');
  btn.disabled=true;btn.textContent='⏳ Running...';
  try{
    const res=await fetch('/step',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tool_name:tool,parameters:params})});
    const data=await res.json();
    if(!data.success){appendTimeline('❌ Server error: '+data.error);return;}
    const state=data.state;const last=state.history[state.history.length-1];stepCount=state.step;
    const outputStr=JSON.stringify(last.output,null,2).split('\\n').map(l=>'    '+l).join('\\n');
    const sign=data.reward>=0?'+':'';
    appendTimeline('── Step '+state.step+' ──────────────────────────\\n'+'  Tool      : '+last.tool+'\\n'+'  Parameters: '+JSON.stringify(last.parameters)+'\\n'+'  Output    :\\n'+outputStr+'\\n'+'  Reward    : '+sign+data.reward.toFixed(2)+'   |   Total: '+state.total_reward.toFixed(2));
    updateStepsBar(stepCount,maxSteps);setRewardEl('stepReward',data.reward);setRewardEl('totalReward',state.total_reward);setRewardEl('normScore',state.normalized_score);
    if(data.done){episodeDone=true;btn.disabled=true;btn.textContent='⚡ Execute Action';setBadge('done');appendTimeline('\\n🏁 Episode complete!  Final reward: '+state.total_reward.toFixed(2)+'  |  Score: '+(state.normalized_score*100).toFixed(1)+'%\\nClick "▶ Start Episode" to run a new episode.');}
  }catch(err){appendTimeline('❌ Network error: '+err.message);}
  finally{if(!episodeDone){btn.disabled=false;btn.textContent='⚡ Execute Action';}}
}
async function runBaseline(){
  const btn=document.getElementById('baselineBtn');
  btn.disabled=true;btn.textContent='⏳ Running all tasks...';
  const out=document.getElementById('baselineOutput');
  out.textContent='Running baseline agent on easy, medium, and hard tasks...\\n';
  try{
    const res=await fetch('/baseline');const data=await res.json();
    if(data.error){out.textContent='❌ Error: '+data.error;return;}
    let display='';
    for(const[,result]of Object.entries(data)){
      const grade=result.normalized_score>=0.8?'🟢':result.normalized_score>=0.5?'🟡':'🔴';
      display+=grade+' Task: '+result.task.toUpperCase()+'\\n'+'   Steps used   : '+result.steps+'\\n'+'   Final reward : '+result.final_reward.toFixed(2)+'\\n'+'   Score        : '+(result.normalized_score*100).toFixed(1)+'%\\n\\n';
    }
    out.textContent=display;
  }catch(err){out.textContent='❌ Network error: '+err.message;}
  finally{btn.disabled=false;btn.textContent='🤖 Run Baseline Solver';}
}
</script>
</body>
</html>
    ...
    try:
        ...
        if _env.current_task is None:
            return jsonify({"success": False,
                            "error": "No episode started. Call /reset first."}), 400
        state, reward, done, info = _env.step(tool_name, parameters)
...
def run_baseline():
    try:
        return jsonify(_baseline_agent.solve())
    except Exception as e:
...
def get_grader():
    try:
        return jsonify({"success": True, "results": _env.get_grading_results()})
    except Exception as e:
...
def main():
    """Called by `serve` console script defined in [project.scripts]."""
